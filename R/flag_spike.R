...
flag_interval_jump = function(df, verbose = FALSE) {
  ...
  axis = time = X = Y = Z = floor_time = NULL
  rm(list = c("X", "Y", "Z", "axis", "time", "floor_time"))
  ...
  xdf = xdf %>%
    dplyr::ungroup() %>%
    dplyr::mutate(floor_time = floor_sec(time)) %>%
    dplyr::left_join(df, by = c("floor_time" = "time")) %>%
    dplyr::mutate(
      flag_interval_jump = dplyr::if_else(is.na(flag_interval_jump), FALSE,
                                          flag_interval_jump)
    ) %>%
    dplyr::select(-floor_time)
  rm(df)
...
flag_spike_second = function(df, spike_size = 11) {
  ...
  time = floor_time = NULL
  rm(list= c("floor_time", "time"))
  # check within 1 second window
  df = df %>%
    dplyr::mutate(floor_time = floor_sec(time)) %>%
    dplyr::group_by(floor_time) %>%
